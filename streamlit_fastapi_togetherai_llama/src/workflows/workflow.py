...
with import_functions():
    from src.functions.function import llm_complete, download_and_clean_html, FunctionInputParams, DownloadInputParams
...
class llm_complete_workflow:
    ...
    async def run(self, input: dict):
        ...
        cleaned_html = await workflow.step(download_and_clean_html, DownloadInputParams(url=url), start_to_close_timeout=timedelta(seconds=120))
